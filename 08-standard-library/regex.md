...
Almost every regex operation revolves around up to four objects:

| Object | Definition | C++ Type |
|---|---|---|
| Target sequence | the text being searched — a range, a null-terminated string, or a `std::string` | iterator pair / `char*` / `std::string` |
| Pattern | the regex itself, built from a string using special grammar (see 2. Pattern Syntax below) | `basic_regex` |
| Matched array | the match information retrieved after a search | `match_results` |
| Replacement string | determines how matches get replaced | plain string, used by `regex_replace` |
...
| \| | alternative (or) |
| `^` | start of line; negation (inside `[...]`) |
| `$` | end of line |

### b. Character Class Shorthands

| Shorthand | Meaning | POSIX Equivalent |
|---|---|---|
| `\d` | decimal digit | `[[:digit:]]` |
| `\l` | lowercase character | `[[:lower:]]` |
| `\s` | space (space, tab, etc.) | `[[:space:]]` |
| `\u` | uppercase character | `[[:upper:]]` |
| `\w` | letter, digit, or underscore | `[[:alnum:]]` |
| `\D` | not `\d` | `[^[:digit:]]` |
| `\L` | not `\l` | `[^[:lower:]]` |
| `\S` | not `\s` | `[^[:space:]]` |
| `\U` | not `\u` | `[^[:upper:]]` |
...
`re(r)`, `re(r, f)`, `re = r`, `assign(r, f)`, `mark_count()`, `flags()`.
...
- `f` refers to `syntax_option_type` — see i. Constants (syntax_option_type) below for the full grammar/variation list.
...
1. It can only be default-constructed, obtained from `std::regex_iterator`, or modified by `std::regex_search`/`std::regex_match`.
...
**Unmatched sub_match (`matched == false`):**

- `str()`/`length()` are well-defined by the standard (a ternary on `matched` short-circuits): they return `""` / `0` regardless of what `first`/`second` actually hold.
- `first`/`second` themselves are **unspecified** by the standard in this case (not undefined behavior — just not pinned down; reading them is safe, but their exact value is implementation-dependent).
- `match_results::position(n)` is **not** guarded by `matched` — it's defined as `distance(prefix().first, (*this)[n].first)` unconditionally. So calling `position(n)` on a non-participating group returns a number derived from that unspecified `first`, which must not be trusted.

### d. regex_match / regex_search

- `regex_match` — attempts to match a regular expression to an **entire** character sequence.
- `regex_search` — attempts to match a regular expression to **any part** of a character sequence; the unmatched text before/after that match can be read via `match_results::prefix()`/`suffix()` (see b. match_results).
...
Not characters, and not `sub_match` directly — `*it` gives read-only access to the current `match_results<BidirIt>`.

**Notes:**

1. Meets the requirements of `LegacyForwardIterator`, with one exception: for dereferenceable `a`, `b` with `a == b`, `*a` and `*b` are **not** guaranteed to be the same object — two iterators can hold separate `match_results` instances describing the same current match. Equality itself compares iterator state (begin/end/pregex/flags/`match[0]`), and `match[0] == rhs.match[0]` is a value comparison of `sub_match` (per c. above), not identity.
...
Specifically, the character-related, locale-sensitive rules used by `basic_regex<CharT, Traits>` — character classification, case handling, locale-sensitive comparison/collation. Same relationship pattern as `iterator_traits`: `CharT → regex_traits<CharT> →` "how should characters of this type be interpreted/compared".
...
Enumerations controlling how a pattern is compiled (`syntax_option_type`) and how a single match/search/replace call behaves (`match_flag_type`).

**syntax_option_type — Grammar (choose one):**
...
**syntax_option_type — Variations (combine via `|`):**
...
**match_flag_type — Matching options:**
...
**match_flag_type — Formatting options (for `regex_replace`):**
...
Applies to `sub_match` (3.c) and `regex_iterator` (3.f): `a == b` compares the represented character sequence / matching state, not `&a == &b`.
...
Don't judge "did this group match anything" purely by whether `str()` is empty — check `sm[n].matched`. E.g. for `(a)?` against input without an `a`, `sm[1].matched == false`; `str()`/`length()` are safely `""`/`0` in that case, but `position(1)` is not guarded and should not be trusted (see c. sub_match → "Unmatched sub_match").
